...
(
    ID          INT AUTO_INCREMENT PRIMARY KEY,
    USER_ID     INT         NOT NULL COMMENT '用户ID',
    BLOCK_ID    INT         NOT NULL COMMENT 'BLOCK ID',
    CONTENT     VARCHAR(50) NOT NULL COMMENT '内容',
    STATUS      BOOLEAN     NOT NULL DEFAULT 0 COMMENT '0是子节点，1是根节点',
    CREATE_TIME DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    UPDATE_TIME DATETIME    NULL     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    CONSTRAINT FK_USER_COMMENT FOREIGN KEY (USER_ID) REFERENCES USER (ID) ON DELETE CASCADE,
    CONSTRAINT FK_BLOCK_COMMENT FOREIGN KEY (BLOCK_ID) REFERENCES BLOCK (ID) ON DELETE CASCADE
);
...
(
    ID        INT AUTO_INCREMENT PRIMARY KEY,
    PARENT_ID INT NOT NULL COMMENT '评论ID',
    CHILD_ID  INT NOT NULL COMMENT '回复ID',
    CONSTRAINT FK_PARENT_COMMENT FOREIGN KEY (PARENT_ID) REFERENCES BLOCK_COMMENT (ID) ON DELETE CASCADE,
    CONSTRAINT FK_CHILD_COMMENT FOREIGN KEY (CHILD_ID) REFERENCES BLOCK_COMMENT (ID) ON DELETE CASCADE
);
